...
namespace RoninVelkoz.Modes
{
    ...
    internal class Combo
    {
        ...
        public static void Execute()
        {
            ...
            if (ComboMenu.GetCheckBoxValue("eUse") && E.IsReady() && etarget.IsValidTarget(SpellsManager.E.Range) && (etarget.HasBuffOfType(BuffType.Slow) || Hitch.IsCC(etarget)))
            {
                E.Cast(etarget);
            }

            if (ComboMenu.GetCheckBoxValue("wUse") && W.IsReady() && wtarget.IsValidTarget(SpellsManager.W.Range)  && !Program.Champion.HasBuff("VelkozR") && wtarget.HasBuffOfType(BuffType.Knockback))
            {
                W.Cast(wtarget);
            }
